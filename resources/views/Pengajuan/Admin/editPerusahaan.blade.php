...
<body>
    ...
    <div class="p-4 sm:ml-64">
        <div class="p-4 border-2 border-gray-200 rounded-lg dark:border-gray-700 mt-14">
            <div class="p-2">
                <h2 class="text-lg font-poppins font-bold"><span>Edit Perusahaan</span></h2>
                <h2 class="text-md font-poppins"><span>Edit perusahaan sesuai dengan data yang sudah disiapkan.</span></h2>
            </div>
            <div class="flex">
                <a href="{{ url()->previous() }}">
            ...
            @if(Auth::user()->role=='kakom')
            <form action="/admin/kakom/daftarperusahaan/update/{{$perusahaan->id_perusahaan}}" method="POST" enctype="multipart/form-data">
            @endif
            @if(Auth::user()->role=='hubin')
            <form action="/admin/hubin/daftarperusahaan/update/{{$perusahaan->id_perusahaan}}" method="POST" enctype="multipart/form-data">
            @endif
            @if(Auth::user()->role=='superadmin')
            <form action="/admin/superadmin/daftarperusahaan/update/{{$perusahaan->id_perusahaan}}" method="POST" enctype="multipart/form-data">
            @endif
                @csrf
                {{ method_field('PUT') }}
                    ...
                    <div class="block bg-transparent w-full overflow-x-auto">
                        ...
                        <div class="flex w-full">
                            <div class="mt-3 w-1/2">
                                <div class="w-full flex flex-col rounded-lg border border-solid">
                                    ...
                                    <div class="w-full rounded-b-lg h-64 p-2">
                                        <input type="file" name="image" onchange="loadFile(event)" value="{{asset('img/perusahaan/'. $perusahaan->gambar_perusahaan)}}">
                                ...
                                <div class="w-full flex border-solid border rounded-lg mt-2">
                                    ...
                                    <div class="w-8/12 p-2 rounded-l-lg">
                                        <input class="h-full w-full outline-none font-poppins" type="text" name="nama_perusahaan" value="{{$perusahaan->nama_perusahaan}}">
                                    </div>
                                </div>
                                <div class="w-full flex border-solid border rounded-lg mt-2">
                                    <div class="w-4/12 bg-gray-200 p-2 rounded-l-lg">
                                    ...
                                    <div class="w-8/12 p-2 rounded-l-lg">
                                        <input class="h-full w-full outline-none font-poppins" type="text" name="alamat_perusahaan" value="{{$perusahaan->alamat_perusahaan}}">
                                    </div>
                                </div>
                            </div>
                            <div class="ml-2 mt-3 w-1/2">
                                <div class="w-full flex flex-col rounded-lg border border-solid">
                                    <div class="w-full bg-gray-200 p-2 rounded-t-lg">
                                        <span class="w-full flex justify-center text-md font-poppins font-semibold text-black">Deskripsi (Tulis "-" jika tidak ingin di isi)</span>
                                    </div>
                                    <div class="w-full rounded-b-lg h-64 p-2">
                                        <textarea class="w-full outline-none font-poppins" name="deskripsi" cols="30" rows="10">{{$perusahaan->deskripsi}}</textarea>
                                    </div>
                                </div> 
                                <div class="w-full flex border-solid border rounded-lg mt-2">
                                    <div class="w-4/12 bg-gray-200 p-2 rounded-l-lg">
