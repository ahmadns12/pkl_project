    ...
    {{-- Content-end --}}
        <div class="p-4 sm:ml-64">
            <div class="p-4 border-2 border-gray-200 rounded-lg dark:border-gray-700 mt-14">
                <div class="p-2">
                    <h2 class="text-lg font-poppins font-bold"><span>Akun Kakom</span></h2>
                    <h2 class="text-md font-poppins"><span>Akun-akun kakom yang terdaftar di aplikasi PKL ini.</span></h2>
                    @if (session('success'))
                ...
                <div class="relative flex flex-col shadow-lg mb-6 rounded-lg p-4">
                    <div class="flex">
                        ...
                        <div class="w-3/12 relative flex flex-col shadow-lg mb-6 rounded-lg p-4 mr-4">
                            <span class="font-poppins font-semibold text-gray-500">Total Akun Kakom</span>
                            <span class="font-poppins font-bold text-[#8AA7FF] text-2xl p-2 text-center">{{$totalAkunkakom}}</span>
                        </div>
                        @if(Auth::user()->role=='superadmin')
                        <div class="w-2/12 relative flex flex-col shadow-lg mb-6 rounded-lg p-4 mr-4 hover:bg-[#8AA7FF] transition ease-linear font-poppins font-semibold text-gray-500 hover:text-white cursor-pointer text-center"  onclick="window.location='{{ route('tambahAkunkakomSuperadmin') }}'">
                            Tambah
                    ...
                    {{-- Search --}}
                    <div class="p-2 mb-2">
                        <label for="search" class="font-poppins font-semibold text-gray-500">Cari Akun kakom:</label>
                        <input type="text" id="search" class="w-full p-2 border border-gray-300 border-solid rounded-lg outline-none font-poppins" placeholder="Nama kakom...">
                    </div>
                    {{-- Search-end --}}
                    <div class="block bg-transparent w-full overflow-x-auto">
                        <table class="w-full mt-2">
                            ...
                            <tbody class="container-akunkakom">
                                @foreach ($dataakunkakom as $item)
                                <tr class="border border-solid rounded-lg text-center">
                                    <td class="text-md px-6 py-3 font-poppins">{{$item->id}}</td>
                                    <td class="text-md font-semibold px-6 py-3 font-poppins">{{ $item->guru->nama_guru }}</td>
                                    <td class="text-md font-semibold px-6 py-3 font-poppins">{{ $item->guru->jurusan->nama_jurusan }}</td>
                                    <td class="text-md px-6 py-3 font-poppins">{{$item->email}}</td>
                                    <td class="text-md px-6 py-3 font-poppins">
                                        @if(Auth::user()->role=='hubin')
                                        <a href="/admin/hubin/akunkakom/edit/{{$item->id_guru}}">
                                        @endif
                                        @if(Auth::user()->role=='kakom')
                                        <a href="/admin/kakom/akunkakom/edit/{{$item->id_guru}}">
                                        @endif
                                        @if(Auth::user()->role=='superadmin')
                                        <a href="/admin/superadmin/akunkakom/edit/{{$item->id_guru}}">
                                        @endif
                                            ...
                                            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                                                ...
                                                <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                                                    ...
                                                    <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                                                        <a href="/admin/superadmin/akunkakom/delete/{{$item->id}}" class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-red-600 text-base font-medium text-white hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 sm:ml-3 sm:w-auto sm:text-sm font-poppins">
                                                            Hapus
                                                        </a>
                    ...
                    <br>
                    {{ $dataakunkakom->links('pagination::tailwind') }}
                </div>
            </div>
        </div>
    {{-- Content-end --}}
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function () {
            ...
            searchInput.addEventListener("input", function () {
                ...
                $.ajax({
                    url: '/admin/cari-akunkakom',
                    type: 'GET',
                    data: { search: searchTerm },
                    success: function (data) {
                        document.querySelector('.container-akunkakom').innerHTML = data;
                    }
                });
            });
        });
