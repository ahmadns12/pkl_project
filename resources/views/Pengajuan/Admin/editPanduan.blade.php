...
<body>
    ...
    <div class="p-4 sm:ml-64">
        <div class="p-4 border-2 border-gray-200 rounded-lg dark:border-gray-700 mt-14">
            <div class="p-2">
                <h2 class="text-lg font-poppins font-bold"><span>Edit Panduan</span></h2>
                <h2 class="text-md font-poppins"><span>Edit panduan sesuai dengan data yang sudah disiapkan.</span></h2>
            </div>
            <div class="flex">
                <a href="{{ url()->previous() }}">
            ...
            @if(Auth::user()->role=='hubin')
            <form action="/admin/hubin/panduan/update/{{$panduan->id_panduan}}" method="POST" enctype="multipart/form-data">
            @endif
            @if(Auth::user()->role=='kakom')
            <form action="/admin/kakom/panduan/update/{{$panduan->id_panduan}}" method="POST" enctype="multipart/form-data">
            @endif
            @if(Auth::user()->role=='superadmin')
            <form action="/admin/superadmin/panduan/update/{{$panduan->id_panduan}}" method="POST" enctype="multipart/form-data">
            @endif
                @csrf
                {{method_field('PUT')}}
                ...
                <div class="relative flex flex-col shadow-lg mb-6 rounded-lg p-4">
                    <div class="block bg-transparent w-full overflow-x-auto">
                        <span class="font-poppins font-semibold">Panduan</span>
                        <div class="flex w-full">
                            <div class="mt-1 w-1/2">
                                <div class="w-full flex flex-col rounded-lg border border-solid">
                                    <div class="w-full bg-gray-200 p-2 rounded-t-lg">
                                        <span class="w-full flex justify-center text-md font-poppins font-semibold text-black">File Panduan</span>
                                    </div>
                                    <div class="w-full rounded-b-lg p-2">
                                        <input type="file" name="file_panduan" value="{{asset('doc/'. $panduan->file_panduan)}}">
                                    </div>
                                </div>
                                <div class="w-full flex border-solid border rounded-lg mt-2">
                                    ...
                                    <div class="w-8/12 p-2 rounded-l-lg">
                                        <input class="h-full w-full outline-none font-poppins" type="text" name="nama_panduan" value="{{$panduan->nama_panduan}}">
                                    </div>
                                </div>
                            </div>
                            <div class="ml-2 mt-1 w-1/2">
                                <div class="w-full flex flex-col rounded-lg border border-solid">
                                    ...
                                    <div class="w-full rounded-b-lg h-24 p-2">
                                        <textarea class="w-full outline-none font-poppins" name="deskripsi" cols="30" rows="3">{{$panduan->deskripsi}}</textarea>
                                    </div>
                                </div> 
                            </div>
                        </div>
