...
<body>
    ...
    <div class="p-4 sm:ml-64">
        <div class="p-4 border-2 border-gray-200 rounded-lg dark:border-gray-700 mt-14">
            <div class="p-2">
                <h2 class="text-lg font-poppins font-bold"><span>Edit Akun Kurikulum</span></h2>
                <h2 class="text-md font-poppins"><span>Edit akun kurikulum sesuai dengan data yang sudah dimasukkan sebelumnya.</span></h2>
            </div>
            <div class="flex">
                <a href="{{ url()->previous() }}">
            ...
            @if(Auth::user()->role=='kakom')
            <form action="/admin/kakom/akunkurikulum/update/{{$akunkurikulum->id}}" method="POST">
            @endif
            @if(Auth::user()->role=='superadmin')
            <form action="/admin/superadmin/akunkurikulum/update/{{$akunkurikulum->id}}" method="POST">
            @endif
                @csrf
                {{method_field('PUT')}}
                <input type="hidden" name="id" value="{{$akunkurikulum->id}}">
                <div class="relative flex flex-col shadow-lg mb-6 rounded-lg p-4">
                    <div class="block bg-transparent w-full overflow-x-auto">
                        <span class="font-poppins font-semibold">Edit Akun Kurikulum</span>
                        <div class="flex w-full">
                            <div class="mt-3 w-1/2">
                                <div class="w-full flex border-solid border rounded-lg">
                                    ...
                                    <div class="w-8/12 p-2 rounded-l-lg">
                                        <input class="h-full w-full outline-none font-poppins" type="text" name="email" value="{{$akunkurikulum->email}}">
                                    </div>
                                </div>
                            ...
                            <div class="ml-2 mt-1 w-1/2">
                                <div class="w-full flex border-solid border rounded-lg mt-2">
                                    ...
                                    <div class="w-8/12 p-2 rounded-l-lg">
                                        <input class="h-full w-full outline-none font-poppins" type="text" name="nama_kurikulum" value="{{$akunkurikulum->nama_kurikulum}}" placeholder="Nama pihak kurikulum...">
                                    </div>
                                </div>
                            </div>
                        </div>
