...
<body>
    ...
    <div class="p-4 sm:ml-64">
        <div class="p-4 border-2 border-gray-200 rounded-lg dark:border-gray-700 mt-14">
            <div class="p-2">
                <h2 class="text-lg font-poppins font-bold"><span>Edit Jurusan</span></h2>
                <h2 class="text-md font-poppins"><span>Edit jurusan sesuai dengan data yang sudah disiapkan.</span></h2>
            </div>
            <div class="flex">
                <a href="{{ url()->previous() }}">
            ...
            <form action="/admin/superadmin/jurusan/update/{{$jurusan->id_jurusan}}" method="POST" enctype="multipart/form-data">
                @csrf
                {{method_field('PUT')}}
                <input type="hidden" name="id">
                <div class="relative flex flex-col shadow-lg mb-6 rounded-lg p-4">
                    <div class="block bg-transparent w-full overflow-x-auto">
                        <span class="font-poppins font-semibold">Jurusan</span>
                            ...
                            <div class="mt-3 w-1/2">
                                <div class="w-full flex flex-col rounded-lg border border-solid">
                                    <div class="w-full bg-gray-200 p-2 rounded-t-lg">
                                        <span class="w-full flex justify-center text-md font-poppins font-semibold text-black">Foto Jurusan</span>
                                    </div>
                                    <div class="w-full rounded-b-lg h-64 p-2">
                                        <input type="file" name="image" onchange="loadFile(event)" value="{{asset('img/jurusan/'. $jurusan->gambar_jurusan)}}">
                                        <div class="w-full flex justify-center items-center">
                                            <img class="mt-2 h-48" id="output" src="{{asset('img/jurusan/'.$jurusan->gambar_jurusan)}}">
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="ml-2 mt-1 w-1/2">
                                <div class="w-full flex border-solid border rounded-lg mt-2">
                                    ...
                                    <div class="w-8/12 p-2 rounded-l-lg">
                                        <input class="h-full w-full outline-none font-poppins" type="text" name="nama_jurusan" value="{{$jurusan->nama_jurusan}}">
                                    </div>
                                </div>
                            </div>
                        </div>
