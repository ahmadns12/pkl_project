...
<body>
    ...
    <div class="p-4 sm:ml-64">
        <div class="p-4 border-2 border-gray-200 rounded-lg dark:border-gray-700 mt-14">
            <div class="p-2">
                <h2 class="text-lg font-poppins font-bold"><span>Edit Akun Siswa</span></h2>
                <h2 class="text-md font-poppins"><span>Edit akun siswa sesuai dengan data siswa yang sudah dimasukkan sebelumnya.</span></h2>
            </div>
            <div class="flex">
                <a href="{{ url()->previous() }}">
            ...
            @if(Auth::user()->role=='kakom')
            <form action="/admin/kakom/akunsiswa/update/{{$akunsiswa->id}}" method="POST">
            @endif
            @if(Auth::user()->role=='superadmin')
            <form action="/admin/superadmin/akunsiswa/update/{{$akunsiswa->id}}" method="POST">
            @endif
                @csrf
                {{method_field('PUT')}}
                <input type="hidden" name="id" value="{{$akunsiswa->id}}">
                <div class="relative flex flex-col shadow-lg mb-6 rounded-lg p-4">
                    <div class="block bg-transparent w-full overflow-x-auto">
                        <span class="font-poppins font-semibold">Edit Akun Siswa</span>
                        <div class="flex w-full">
                            ...
                            <div class="ml-2 mt-1 w-1/2">
                                <div class="w-full flex border-solid border rounded-lg mt-2">
                                    ...
                                    <div class="w-8/12 p-2 rounded-l-lg">
                                        <input class="h-full w-full outline-none font-poppins" type="text" name="angkatan" value="{{$akunsiswa->angkatan}}">
                                    </div>
                                </div>
                                <div class="w-full flex border-solid border rounded-lg mt-2">
                                    <div class="w-4/12 bg-gray-200 p-2 rounded-l-lg">
                                    ...
                                    <div class="w-8/12 p-2 rounded-l-lg">
                                        <input class="h-full w-full outline-none font-poppins" type="text" name="id_siswa" value="{{ $siswa->nama_siswa }}" disabled>
